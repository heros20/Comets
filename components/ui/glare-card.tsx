import { cn } from "@/lib/utils"; // ou remplace par classnames si tu préfères
  ...
  return (
    <div
      ...
      onPointerMove={(e) => {
        ...
        const percent = { x: (100 / rect.width) * pos.x, y: (100 / rect.height) * pos.y };
        const delta = { x: percent.x - 50, y: percent.y - 50 };
        state.current.background.x = 50 + percent.x / 4 - 12.5;
        state.current.background.y = 50 + percent.y / 3 - 16.67;
        state.current.rotate.x = -(delta.x / 3.5) * rotateFactor;
        state.current.rotate.y = (delta.y / 2) * rotateFactor;
        state.current.glare.x = percent.x;
        state.current.glare.y = percent.y;
        updateStyles();
      }}
      onPointerEnter={() => {
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
