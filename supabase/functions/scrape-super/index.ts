...
serve(async (req) => {
  try {
    ...
    // ===== CLASSEMENT SCRAPE =====
    const classementRes = await fetch(CLASSEMENT_URL, { headers });
    if (!classementRes.ok) throw new Error("Erreur fetch classement FFBS " + classementRes.status);
    const classementHtml = await classementRes.text();
    const $c = cheerio.load(classementHtml);

    const tabs: string[] = [];
    $c("ul.nav-tabs li a").each((i, el) => tabs.push($c(el).text().trim()));

    const standings: any[] = [];
    $c(".tab-content .tab-pane").each((i, tabPane) => {
      const tabRows: any[] = [];
      $c(tabPane).find("table.standings-print tr").slice(1).each((j, row) => {
        const cols = $c(row).find("td");
        if (cols.length === 8) {
          const teamCell = $c(cols[2]);
          ...
          const logo = $c(cols[1]).find("img").attr("src") || null;
          const teamLink = teamCell.find("a").attr("href") || null;
          tabRows.push({
            rank: parseInt($c(cols[0]).text().trim(), 10),
            abbreviation: abbr,
            name,
            logo,
            team_url: teamLink,
            W: parseInt($c(cols[3]).text().trim(), 10),
            L: parseInt($c(cols[4]).text().trim(), 10),
            T: parseInt($c(cols[5]).text().trim(), 10),
            PCT: $c(cols[6]).text().trim(),
            GB: $c(cols[7]).text().trim()
          });
        }
      });
    ...
    const year = $c("title").text().match(/\d{4}/)?.[0] || "2025";

    // Delete + insert new classement
    await supabase.from("classement_normandie").delete().neq('id', 0);
    const { error: classementError } = await supabase.from("classement_normandie").insert([{ data: { tabs, standings, year } }]);
    if (classementError) throw new Error("Erreur insertion classement : " + classementError.message);

    // ===== TEAM SCRAPE =====
    const teamRes = await fetch(TEAM_URL, { headers });
    if (!teamRes.ok) throw new Error("Erreur fetch équipe FFBS " + teamRes.status);
    ...
    if (roster.length) {
      playerInsert = await supabase.from('players').upsert(roster, { onConflict: ['player_link'] });
    }
    if (games.length) {
      gamesInsert = await supabase.from('games').upsert(games, { onConflict: ['team_abbr', 'game_number'] });
    }
    ...
    return new Response(JSON.stringify({
      ok: true,
      message: "Super scrape terminé !",
      tabs,
      year,
      standingsScraped: standings.map(t => t.length),
      teamName,
